...
/* Keep sidebar and modal clickable */
...
/* Only blur the dashboard (sidebar + main) when search modal is open */
body.modal-open .sidebar,
body.modal-open .main {
    filter: blur(5px);
    pointer-events: none;
...
function toggleSidebar() {
    if(window.innerWidth <= 900){
        ...
        sidebar.classList.remove('folded'); // remove desktop folded
    } else {
        // Desktop: fold/unfold
        sidebar.classList.toggle('folded');
        sidebar.classList.remove('show'); // remove mobile show
    }
}
...
// Click outside to close sidebar only on mobile
...
window.addEventListener('resize', () => {
    if(window.innerWidth > 900){
        sidebar.classList.remove('show'); // always show folded sidebar
    }
});
...
if(searchInput){ // only attach if element exists
    searchInput.addEventListener('keypress', function(e) {
        if(e.key === 'Enter'){
            ...
            fetch(`../public/search_results.php?query=${encodeURIComponent(query)}`)
                .then(resp => resp.ok ? resp.text() : Promise.reject('Network error'))
                .then(html => {
                    searchModalBody.innerHTML = html;
                })
                .catch(err => {
                    searchModalBody.innerHTML = `<p style="color:#c00">Failed to load results. <a href="../public/search_results.php?query=${encodeURIComponent(query)}" target="_blank">Open full results</a></p>`;
                    console.error(err);
                });
        }
    });
...
searchInput.addEventListener('keypress', function(e) {
    if(e.key === 'Enter') {
        ...
        fetch(`../public/search_results.php?query=${encodeURIComponent(query)}`)
            .then(resp => resp.ok ? resp.text() : Promise.reject('Network error'))
            .then(html => { searchModalBody.innerHTML = html; })
            .catch(err => {
                searchModalBody.innerHTML = `<p style="color:#c00">Failed to load results. <a href="../public/search_results.php?query=${encodeURIComponent(query)}" target="_blank">Open full results</a></p>`;
                console.error(err);
            });
