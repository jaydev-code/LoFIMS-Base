...
try {
    ...
    // Get stats
    $total_found = $pdo->prepare("SELECT COUNT(*) FROM found_items WHERE user_id = ?");
    $total_found->execute([$_SESSION['user_id']]);
    $total_found = $total_found->fetchColumn();
    
    $unclaimed = $pdo->prepare("SELECT COUNT(*) FROM found_items WHERE user_id = ? AND status = 'Found'");
    $unclaimed->execute([$_SESSION['user_id']]);
    $unclaimed = $unclaimed->fetchColumn();
    
    $claimed = $pdo->prepare("SELECT COUNT(*) FROM found_items WHERE user_id = ? AND status = 'Claimed'");
    $claimed->execute([$_SESSION['user_id']]);
    $claimed = $claimed->fetchColumn();
    
    $returned = $pdo->prepare("SELECT COUNT(*) FROM found_items WHERE user_id = ? AND status = 'Returned'");
    $returned->execute([$_SESSION['user_id']]);
    $returned = $returned->fetchColumn();
...
<head>
    ...
    <title>Found Items - LoFIMS</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <link rel="stylesheet" href="css/styles.css">
    <style>
        /* Additional styles for found items page */
        .page-title {
            font-size: 28px;
            color: #1e293b;
            margin-bottom: 20px;
...
<div class="main">
    ...
    <div class="page-content">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <div class="stat-number"><?php echo $unclaimed; ?></div>
                <div class="stat-label">Awaiting Claim</div>
            </div>
            <div class="stat-card">
                <div class="stat-number"><?php echo $claimed; ?></div>
                <div class="stat-label">Claimed Items</div>
            </div>
            <div class="stat-card">
                <div class="stat-number"><?php echo $returned; ?></div>
        ...
        <div class="action-bar">
            <div class="action-buttons">
                <a href="items/found/add.php" class="btn btn-primary" style="background: #10b981;">
            ...
            <div style="display: flex; gap: 10px;">
                <input type="text" id="searchItems" placeholder="Search found items..." 
        ...
        <div class="table-responsive">
            <table class="data-table" id="foundItemsTable">
                <thead>
                    <tr>
                        <th>Item Name</th>
                        <th>Category</th>
                ...
                <tbody>
                    ...
                    <tr data-status="<?php echo $item['status']; ?>">
                        ...
                        <td>
                            <?php echo date('M d, Y', strtotime($item['date_found'])); ?><br>
                            <small style="color: #94a3b8;"><?php echo date('h:i A', strtotime($item['created_at'])); ?></small>
                        </td>
                        <td><?php echo htmlspecialchars($item['place_found']); ?></td>
                        <td>
                        ...
                        <td>
                            ...
                            <span class="badge" style="background: #3b82f6; color: white; padding: 4px 8px; border-radius: 12px;">
                        ...
                        <td class="table-actions">
                            <a href="items/found/view.php?id=<?php echo $item['found_id']; ?>" 
                               class="btn btn-primary btn-sm">
                                <i class="fas fa-eye"></i>
                            </a>
                            <a href="items/found/edit.php?id=<?php echo $item['found_id']; ?>" 
                               class="btn btn-success btn-sm">
                                <i class="fas fa-edit"></i>
                            </a>
                            <?php if($item['status'] == 'Found'): ?>
                            <button class="btn btn-warning btn-sm" 
                                    onclick="markAsClaimed(<?php echo $item['found_id']; ?>)">
                                <i class="fas fa-handshake"></i>
                            </button>
                            <?php endif; ?>
                        </td>
        ...
        <div class="empty-state">
            ...
            <p>You haven't added any found items yet. Help others by reporting items you've found.</p>
            <a href="items/found/add.php" class="btn btn-primary" style="background: #10b981;">
                <i class="fas fa-plus"></i> Add Your First Found Item
            </a>
        </div>
        <?php endif; ?>
    </div>
...
function filterTable() {
    ...
    const rows = document.querySelectorAll('#foundItemsTable tbody tr');
...
function markAsClaimed(foundId) {
    if (confirm('Mark this item as claimed? This will change its status to "Claimed".')) {
        // This would need an actual endpoint to handle the status update
        fetch(`items/found/update_status.php?id=${foundId}&status=Claimed`)
